...
@Component({
  ...
  styleUrls: ['./trademark-list.component.css']
})
export class TrademarkListComponent implements OnInit {
  ...
  setMatTableDataSrouce() {
    this.dataSource = new MatTableDataSource(this.convertTrademarksToTableData());
  }

  convertTrademarksToTableData() {
    let rows = new Array<TableData>();
    if (this.trademarks !== undefined)
      if (!Array.isArray(this.trademarks)) {
        let metaData = this.trademarks.MetaData;
        rows.push({
